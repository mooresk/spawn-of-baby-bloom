...
load([filepath 'Shimada\Data\summary_19-23Hake_biovolume'],'PB');
PB(~(PB.DT.Year==yr),:)=[]; %select year of data
names=PB.Properties.VariableNames(8:27);
bvmL=timetable2table(PB(:,8:27));
bvmL(:,1)=[];
...
if fprint
    exportgraphics(fig,[filepath 'plotting-quickstart\phytoplankton_composition\Figs\FxPhytoCommunity_CCS_' num2str(yr) '.png'],'Resolution',100)    
end
hold off 
